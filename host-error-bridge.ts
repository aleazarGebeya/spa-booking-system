...
(() => {
  ...
  const isPluginError = (message: string, stack: string, source?: string): boolean => {
    ...
    const isExtensionHydration = 
      ...
      (/data-.*-root|float.*left|position.*relative.*height.*0px/i.test(combinedText));

    return pluginPatterns.some(pattern => pattern.test(combinedText)) || isExtensionHydration;
  };
  ...
  const isSandboxError = (message: string, stack: string, source?: string): boolean => {
    ...
    const hasSandboxSource = 
      ...
      stack && (
        /\/app\/|\/src\/|\/components\/|\/pages\/|\/lib\/|\/utils\//i.test(stack) ||
    ...
    const hasErrorPattern = sandboxErrorPatterns.some(pattern => pattern.test(combinedText));

    // Must have both sandbox source and error pattern, or be a clear sandbox error
    return (hasSandboxSource && hasErrorPattern) || 
  ...
  const handleSandboxError = (event: ErrorEvent | PromiseRejectionEvent) => {
    try {
      ...
      if (isSandboxError(message, stack, source)) {
        postError({
          type: event instanceof ErrorEvent ? 'javascript_error' : 'promise_rejection',
          message,
          stack,
          source,
          lineno,
  ...
  window.addEventListener('unhandledrejection', handleSandboxError, { capture: true });
